...
type Tab = "staff" | "teams" | "leave" | "roles" | "plan" | "future";
type LeaveStatus = "Pending" | "Approved" | "Rejected";
type LeaveRequest = { id: number; staffMemberId: number; startDate: string; endDate: string; reason: string; status: LeaveStatus; createdAt: string };
...
export default function CustomerStaff() {
  ...
  const filteredMembers = useMemo(() => {
    ...
    return workspace.members.filter(member => [member.firstName, member.lastName, member.email, member.roleName, member.status, member.skills, member.serviceArea].join(" ").toLowerCase().includes(query));
  ...
  async function saveTeam() {
    ...
    try {
      setSaving(true);
      setError("");
      const request = { name: teamName, description: teamDescription, colour: teamColour, defaultJobType: teamDefaultJobType, serviceArea: teamServiceArea, workingHours: teamWorkingHours };
      const updatedWorkspace = editingTeamId ? await customerStaffService.updateTeam(editingTeamId, request) : await customerStaffService.createTeam(request);
  ...
  async function inviteMember() {
    ...
    try {
      setSaving(true);
      setError("");
      const response = await customerStaffService.createMember({ firstName: memberFirstName, lastName: memberLastName, email: memberEmail, phone: memberPhone, roleName: memberRole, permissionPresetName: memberRole, teamIds: memberTeamIds, skills: memberSkills, serviceArea: memberServiceArea, workingHours: memberWorkingHours, calendarColour: memberColour });
  ...
  async function updateMember(member: CustomerStaffMember, patch: Partial<CustomerStaffMember>) {
    try {
      ...
      setWorkspace(await customerStaffService.updateMember(member.id, { firstName: updated.firstName, lastName: updated.lastName, email: updated.email, phone: updated.phone, roleName: updated.roleName, status: updated.status, permissionPresetName: updated.permissionPresetName, teamIds: updated.teamIds, skills: updated.skills, serviceArea: updated.serviceArea, workingHours: updated.workingHours, calendarColour: updated.calendarColour, isTwoFactorRequired: updated.isTwoFactorRequired }));
  ...
  function addLeaveRequest() {
    ...
    const request: LeaveRequest = { id: Date.now(), staffMemberId: staffId, startDate: leaveStartDate, endDate: leaveEndDate, reason: leaveReason.trim(), status: "Pending", createdAt: new Date().toISOString() };
    const next = [request, ...leaveRequests];
  ...
  function updateLeaveStatus(id: number, status: LeaveStatus) {
    const next = leaveRequests.map(request => request.id === id ? { ...request, status } : request);
    saveLeaveRequests(next);
    setLeaveRequests(next);
  }
        ...
        <div className="max-w-7xl">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Invite workers, build flexible teams, assign roles, plan job calendars, and keep customer company staff completely separate from TradeLike internal staff.</p>
            </div>
          ...
          {loading ? <p className="mt-8 text-sm text-slate-500">Loading...</p> : (
            <div className="mt-6">
              {activeTab === "staff" && <div className="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
                <Panel title="Invite worker">
                  ...
                  <div className="grid gap-3">
                    ...
                    <Select value={memberColour} onChange={setMemberColour}>{teamColours.map(colour => <option key={colour.value} value={colour.value}>{colour.label}</option>)}</Select>
                ...
                <Panel title="Company staff">
                  ...
                  <div className="mt-4 grid gap-3">
                    {filteredMembers.map(member => <StaffCard key={member.id} member={member} teams={workspace.teams} leaveRequests={leaveRequests} selected={selectedMemberId === member.id} onSelect={() => setSelectedMemberId(selectedMemberId === member.id ? null : member.id)} onPasswordReset={() => requestPasswordReset(member)} onStatusChange={status => updateMember(member, { status })} onRoleChange={role => updateMember(member, { roleName: role, permissionPresetName: role })} onTeamsChange={teamIds => updateMember(member, { teamIds })} roleOptions={workspace.roleOptions} />)}
                    {filteredMembers.length === 0 && <p className="text-sm text-slate-500">No staff found.</p>}
                  </div>
                  {selectedMember && <DetailBox title={`${selectedMember.firstName} ${selectedMember.lastName}`} items={[`Role: ${selectedMember.roleName}`, `Status: ${selectedMember.status}`, `Email: ${selectedMember.email}`, `Phone: ${selectedMember.phone || "Not set"}`, `Working hours: ${selectedMember.workingHours || "Not set"}`, `Upcoming leave: ${formatLeaveSummary(nextLeaveForMember(leaveRequests, selectedMember.id))}`]} />}
                </Panel>
              </div>}

              {activeTab === "teams" && <div className="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
                <Panel title={editingTeamId ? "Edit team" : "Create team"}>
                  ...
                  <div className="grid gap-3">
                    ...
                    <Select value={teamColour} onChange={setTeamColour}>{teamColours.map(colour => <option key={colour.value} value={colour.value}>{colour.label}</option>)}</Select>
                ...
                <Panel title="Teams">
                  <div className="grid gap-3 md:grid-cols-2">
                    {workspace.teams.map(team => <TeamCard key={team.id} team={team} members={workspace.members.filter(member => member.teamIds.includes(team.id))} selected={selectedTeamId === team.id} onSelect={() => setSelectedTeamId(selectedTeamId === team.id ? null : team.id)} onDelete={() => deleteTeam(team)} onEdit={() => startEditTeam(team)} />)}
                  </div>
                  {selectedTeam && <DetailBox title={selectedTeam.name} items={[`Colour: ${getTeamColourLabel(selectedTeam.colour)}`, `Area: ${selectedTeam.serviceArea || "Not set"}`, `Hours: ${selectedTeam.workingHours || "Not set"}`, `Default job type: ${selectedTeam.defaultJobType || "Not set"}`, `Members: ${workspace.members.filter(member => member.teamIds.includes(selectedTeam.id)).map(member => `${member.firstName} ${member.lastName}`).join(", ") || "None"}`]} />}
                </Panel>
              </div>}

              {activeTab === "leave" && <Panel title="Annual leave">
                <div className="grid gap-3 md:grid-cols-5">
                  <select value={leaveStaffId || String(selectedMemberId ?? "")} onChange={event => setLeaveStaffId(event.target.value)} className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-600"><option value="">Choose staff</option>{workspace.members.filter(member => member.status !== "Left").map(member => <option key={member.id} value={member.id}>{member.firstName} {member.lastName}</option>)}</select>
                  ...
                  <button type="button" onClick={addLeaveRequest} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">Add leave</button>
                </div>
                <div className="mt-5 grid gap-3">
                  {leaveRequests.map(request => { const member = workspace.members.find(item => item.id === request.staffMemberId); return <div key={request.id} className="rounded-lg border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold text-slate-900">{member ? `${member.firstName} ${member.lastName}` : "Unknown staff"}</p><p className="text-sm text-slate-600">{formatDate(request.startDate)} → {formatDate(request.endDate)} · {request.reason || "Annual leave"}</p></div><Select value={request.status} onChange={value => updateLeaveStatus(request.id, value as LeaveStatus)}><option value="Pending">Pending</option><option value="Approved">Approved</option><option value="Rejected">Rejected</option></Select></div></div>; })}
                  {leaveRequests.length === 0 && <p className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500">No annual leave booked yet.</p>}
                </div>
              </Panel>}
...
function StaffCard({ member, teams, leaveRequests, selected, onSelect, onPasswordReset, onStatusChange, onRoleChange, onTeamsChange, roleOptions }: { member: CustomerStaffMember; teams: CustomerTeam[]; leaveRequests: LeaveRequest[]; selected: boolean; onSelect: () => void; onPasswordReset: () => void; onStatusChange: (status: CustomerStaffMember["status"]) => void; onRoleChange: (role: string) => void; onTeamsChange: (ids: number[]) => void; roleOptions: string[] }) {
  const leave = nextLeaveForMember(leaveRequests, member.id);
  return <div onClick={onSelect} className={`cursor-pointer rounded-xl border p-4 transition ${selected ? "border-blue-500 bg-blue-50" : "border-slate-200 bg-slate-50 hover:bg-white"}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold text-slate-900">{member.firstName} {member.lastName}</p><p className="text-sm text-slate-600">{member.email}</p><p className="mt-1 text-xs font-semibold text-blue-700">{member.roleName} · {member.status}</p>{leave && <p className="mt-1 text-xs font-semibold text-amber-700">Leave: {formatDate(leave.startDate)} → {formatDate(leave.endDate)} ({leave.status})</p>}</div><div className="flex flex-wrap gap-2" onClick={event => event.stopPropagation()}><button type="button" onClick={onPasswordReset} className="rounded-lg border border-slate-300 px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-white">Reset password</button><Select value={member.status} onChange={value => onStatusChange(value as CustomerStaffMember["status"])}>{statuses.map(status => <option key={status} value={status}>{status}</option>)}</Select></div></div><div className="mt-3 grid gap-2 md:grid-cols-2" onClick={event => event.stopPropagation()}><Select value={member.roleName} onChange={onRoleChange}>{roleOptions.map(role => <option key={role} value={role}>{role}</option>)}</Select><TeamPicker teams={teams} selectedIds={member.teamIds} onChange={onTeamsChange} compact /></div><div className="mt-3 grid gap-2 text-xs text-slate-600 md:grid-cols-3"><span>Skills: {member.skills || "Not set"}</span><span>Area: {member.serviceArea || "Not set"}</span><span>Reset: {member.resetPasswordRequestedAt ? formatDate(member.resetPasswordRequestedAt) : "No request"}</span></div></div>;
}
...
function TeamCard({ team, members, selected, onSelect, onDelete, onEdit }: { team: CustomerTeam; members: CustomerStaffMember[]; selected: boolean; onSelect: () => void; onDelete: () => void; onEdit: () => void }) {
  ...
  return <div onClick={onSelect} className={`cursor-pointer rounded-xl border p-4 transition ${selected ? "ring-2 ring-blue-400" : ""}`} style={{ borderColor: colour.hex, backgroundColor: colour.softHex }}><div className="flex justify-between gap-4"><div><p className="font-bold text-slate-900">{team.name}</p><p className="text-sm text-slate-600">{team.description || "No description"}</p><p className="mt-1 text-xs font-bold" style={{ color: colour.hex }}>{colour.label}</p></div><div className="flex flex-col gap-2" onClick={event => event.stopPropagation()}><button type="button" onClick={onDelete} className="h-fit rounded border border-red-200 bg-white px-2 py-1 text-xs font-semibold text-red-600 hover:bg-red-50">Delete</button><button type="button" onClick={onEdit} className="h-fit rounded border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">Edit</button></div></div><div className="mt-3 text-xs text-slate-600"><p>{members.length} member{members.length === 1 ? "" : "s"}</p><p>Area: {team.serviceArea || "Not set"}</p><p>Hours: {team.workingHours || "Not set"}</p></div></div>;
}

function DetailBox({ title, items }: { title: string; items: string[] }) {
  return <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50 p-4"><p className="font-bold text-slate-900">{title}</p><div className="mt-2 grid gap-1 text-sm text-slate-700">{items.map(item => <span key={item}>{item}</span>)}</div></div>;
}

function PlanPanel({ workspace, currentUserCount }: { workspace: CustomerStaffWorkspace; currentUserCount: number }) {
  const items = [["Max users", workspace.entitlements.maxUsers ?? "Unlimited"], ["Teams", workspace.entitlements.teamsEnabled ? "Included" : "Upgrade"], ["Staff scheduling", workspace.entitlements.staffSchedulingEnabled ? "Included" : "Upgrade"], ["Advanced permissions", workspace.entitlements.advancedPermissionsEnabled ? "Included" : "Upgrade"], ["Reporting", workspace.entitlements.reportingEnabled ? "Included" : "Upgrade"], ["API access", workspace.entitlements.apiAccessEnabled ? "Included" : "Upgrade"]];
...
function Input({ value, onChange, placeholder, type = "text" }: { value: string; onChange: (value: string) => void; placeholder: string; type?: string }) {
  return <input type={type} value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-600" />;
}
...
function loadLeaveRequests() {
  try { return JSON.parse(localStorage.getItem(leaveStorageKey) ?? "[]") as LeaveRequest[]; } catch { return []; }
}

function saveLeaveRequests(requests: LeaveRequest[]) {
  localStorage.setItem(leaveStorageKey, JSON.stringify(requests));
}

function nextLeaveForMember(requests: LeaveRequest[], staffMemberId: number) {
  const now = new Date();
  now.setHours(0, 0, 0, 0);
  return requests.filter(request => request.staffMemberId === staffMemberId && request.status !== "Rejected" && new Date(request.endDate) >= now).sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime())[0];
}

function formatLeaveSummary(request?: LeaveRequest) {
  return request ? `${formatDate(request.startDate)} → ${formatDate(request.endDate)} (${request.status})` : "None";
}

...
import StudioAuditLogCenter from "./StudioAuditLogCenter";
import StudioBillingPlans from "./StudioBillingPlans";
import StudioCustomerHealth from "./StudioCustomerHealth";
...
export default function AdminPortal() {
  ...
  const canSeeBilling =
    ...
    Boolean(currentUser?.canManageSubscriptions) ||
    Boolean(currentUser?.canManageDiscounts) ||
    Boolean(currentUser?.canManageFreeMonths);
  const canSeeSupportNotes =
    isDirector ||
    Boolean(currentUser?.canManageAccounts) ||
    Boolean(currentUser?.canViewCustomerNotes) ||
    Boolean(currentUser?.canEditCustomerNotes);
  ...
  const canSeeSettings =
    ...
    Boolean(currentUser?.canManageSecurity) ||
    Boolean(currentUser?.canManageStaff);
  const canLoadCustomerData = canSeeAccounts || canSeeBilling || canSeeSupportNotes;

  const availableSections = useMemo(() => {
    const sections: AdminSection[] = [];

    if (canSeeAccounts || canSeeStaff || canSeeAuditLogs || canSeeBilling) {
    ...
    return sections;
  }, [canSeeAccounts, canSeeAuditLogs, canSeeBilling, canSeeDiagnostics, canSeeSettings, canSeeStaff, canSeeSupportNotes]);
  ...
  async function loadTenantUsers(userId: number) {
    try {
      setLoadingTenantUsers(true);
      setTenantUsers(await adminService.getCustomerUsers(userId));
    } catch {
      setTenantUsers([]);
    } finally {
  ...
  function selectUser(user: AdminUser) {
    ...
    loadTenantUsers(user.id);
  }

  function openCustomer(user: AdminUser) {
    if (!canSeeAccounts) {
      showError("You do not have permission to open customer account details.");
      return;
    }
  ...
  function openStaff(user: AdminUser) {
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden bg-slate-950 text-slate-100">
      ...
      <section className="mx-auto max-w-7xl px-6 py-8">
        ...
        {canLoadCustomerData && (
          <StudioGlobalSearch users={users} query={globalSearch} onQueryChange={setGlobalSearch} onOpenCustomer={openCustomer} />
        )}
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-[260px_1fr]">
          <aside className="h-fit rounded-xl border border-slate-800 bg-slate-900 p-4">
            ...
            <div className="mt-3 flex flex-col gap-2">
              {availableSections.includes("overview") && <AdminNavButton active={activeSection === "overview"} onClick={() => setActiveSection("overview")}>Overview</AdminNavButton>}
              {canSeeAccounts && <AdminNavButton active={activeSection === "accounts"} onClick={() => setActiveSection("accounts")}>Customers</AdminNavButton>}
              {canSeeAccounts && <AdminNavButton active={activeSection === "health"} onClick={() => setActiveSection("health")}>Customer Health</AdminNavButton>}
              {canSeeBilling && <AdminNavButton active={activeSection === "billing"} onClick={() => setActiveSection("billing")}>Billing & Plans</AdminNavButton>}
              {canSeeStaff && <AdminNavButton active={activeSection === "staff"} onClick={() => setActiveSection("staff")}>Staff</AdminNavButton>}
              {canSeeStaff && <AdminNavButton active={activeSection === "permissions"} onClick={() => setActiveSection("permissions")}>Permissions</AdminNavButton>}
              {canSeeAuditLogs && <AdminNavButton active={activeSection === "audit"} onClick={() => setActiveSection("audit")}>Audit Log</AdminNavButton>}
              {canSeeSupportNotes && <AdminNavButton active={activeSection === "notes"} onClick={() => setActiveSection("notes")}>Support Notes</AdminNavButton>}
              {canSeeDiagnostics && <AdminNavButton active={activeSection === "diagnostics"} onClick={() => setActiveSection("diagnostics")}>Diagnostics</AdminNavButton>}
              {canSeeSettings && <AdminNavButton active={activeSection === "settings"} onClick={() => setActiveSection("settings")}>Settings</AdminNavButton>}
            </div>
          </aside>

          <div>
            {activeSection === "overview" && <AdminDashboard users={users} staffUsers={staffUsers} auditLogs={auditLogs} onOpenAccounts={() => setActiveSection(canSeeAccounts ? "accounts" : canSeeBilling ? "billing" : "overview")} onOpenAudit={() => setActiveSection(canSeeAuditLogs ? "audit" : "overview")} />}

            {activeSection === "health" && canSeeAccounts && (
              <StudioCustomerHealth users={users} onOpenCustomer={openCustomer} />
            )}

            {activeSection === "billing" && canSeeBilling && (
              <StudioBillingPlans users={users} onOpenCustomer={openCustomer} />
            )}
                      ...
                      <div className="grid gap-6 xl:grid-cols-2">
                        <form onSubmit={handleSaveAccount} className="rounded-xl border border-slate-800 bg-slate-900 p-5 shadow-sm">
                          ...
                          <div className="mt-5 grid gap-4">
                            ...
                            <Field label="Discount type"><DarkSelect value={discountType} onChange={value => setDiscountType(value as AdminDiscountType)}>{discountTypes.map(type => <option key={type} value={type}>{type === "None" ? "No discount" : type === "Amount" ? "GBP amount" : "% percentage"}</option>)}</DarkSelect></Field>
                            <Field label={discountType === "Percentage" ? "Discount value (%)" : "Discount value (GBP)"}><DarkInput value={discountValue} onChange={setDiscountValue} type="number" min="0" max={discountType === "Percentage" ? 100 : undefined} step="1" disabled={discountType === "None"} /></Field>
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="rounded-xl border border-slate-800 bg-slate-900 p-5 shadow-sm"><h2 className="text-lg font-bold text-white">Account snapshot</h2><div className="mt-4 grid gap-3 text-sm text-slate-400"><SnapshotRow label="User ID" value={String(selectedUser.id)} /><SnapshotRow label="Tenant ID" value={String(selectedUser.tenantId ?? selectedUser.id)} /><SnapshotRow label="Last login" value={selectedUser.lastLoginAt ? formatDateTime(selectedUser.lastLoginAt) : "Never"} /><SnapshotRow label="Trial ends" value={selectedUser.trialEndsAt ? formatDateTime(selectedUser.trialEndsAt) : "Not set"} /><SnapshotRow label="Free months expiry" value={selectedUser.freeMonthsExpireAt ? formatDateTime(selectedUser.freeMonthsExpireAt) : "Not set"} /><SnapshotRow label="Onboarding email" value={selectedUser.onboardingEmailSentAt ? formatDateTime(selectedUser.onboardingEmailSentAt) : "Not sent"} /><SnapshotRow label="Verification email" value={selectedUser.emailVerificationSentAt ? formatDateTime(selectedUser.emailVerificationSentAt) : "Never"} /></div></div>
                          <div className="rounded-xl border border-slate-800 bg-slate-900 shadow-sm"><div className="border-b border-slate-800 px-5 py-4"><h2 className="text-lg font-bold text-white">Tenant users</h2><p className="mt-1 text-sm text-slate-400">Customer users in this tenant stay separate from internal Studio staff.</p></div>{loadingTenantUsers ? <div className="p-5 text-sm text-slate-400">Loading...</div> : tenantUsers.length === 0 ? <div className="p-5 text-sm text-slate-400">No tenant users found.</div> : <div className="max-h-[280px] divide-y divide-slate-800 overflow-y-auto">{tenantUsers.map(user => <div key={user.id} className="grid gap-3 px-5 py-4 md:grid-cols-[minmax(0,1fr)_120px]"><div className="min-w-0"><p className="truncate text-sm font-semibold text-white">{user.fullName || user.email}</p><p className="mt-1 truncate text-xs text-slate-500">{user.email}</p></div><Badge>{user.role}</Badge></div>)}</div>}</div>
            ...
            {activeSection === "permissions" && canSeeStaff && (
              <StudioPermissionsMatrix staffUsers={staffUsers} onOpenStaff={openStaff} />
            )}

            {activeSection === "notes" && canSeeSupportNotes && (
              <StudioSupportNotes users={users} onOpenCustomer={openCustomer} onCustomerUpdated={upsertUser} />
            )}

            {activeSection === "diagnostics" && canSeeDiagnostics && (
              <StudioDiagnostics currentUser={currentUser} />
            )}

            {activeSection === "settings" && canSeeSettings && (
              <StudioSettings currentUser={currentUser} />
            )}

            {activeSection === "audit" && canSeeAuditLogs && (
              <StudioAuditLogCenter auditLogs={auditLogs} loading={loadingAuditLogs} search={auditSearch} onSearchChange={setAuditSearch} onSearch={() => loadAuditLogs(auditSearch)} />
            )}
          </div>
        </div>
      </section>

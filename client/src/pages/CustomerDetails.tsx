...
export default function CustomerDetails() {
  ...
  const timelineItems = useMemo<TimelineItem[]>(() => {
    const quoteItems = quotes.map(quote => ({ id: `quote-${quote.id}`, date: quote.createdAt, title: `Quote ${quote.status.toLowerCase()}`, body: `${quote.title} Â· ${formatCurrency(quote.total)}`, href: `/quotes/${quote.id}` }));
    const jobItems = jobs.map(job => ({ id: `job-${job.id}`, date: job.scheduledDate, title: `Job ${job.status.toLowerCase()}`, body: `${job.jobTitle} Â· ${job.priority} priority`, href: `/jobs/${job.id}` }));
    const invoiceItems = invoices.map(invoice => ({ id: `invoice-${invoice.id}`, date: invoice.createdAt, title: `Invoice ${invoice.status.toLowerCase()}`, body: `${invoice.invoiceNumber} Â· ${formatCurrency(invoice.total)}`, href: "/invoices" }));
  ...
  async function handleSave(event: FormEvent) {
    ...
    if (!isValidOptionalUkPhone(form.phone)) {setError("Client phone number must be 11 digits, for example 07981 125031.");setMessage("");return;}
  ...
  return (
    <main className="flex min-h-screen bg-slate-50">
      ...
      <section className="flex-1 p-10">
        <div className="mb-6"><Link to="/customers" className="text-sm font-medium text-blue-600 hover:underline">â† Back to Clients</Link></div>

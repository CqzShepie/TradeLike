...
export default function JobDetailsAssignmentPanel({ job }: Props) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-bold text-slate-900">Staff and Team</h2><p className="mt-1 text-sm text-slate-500">Assign a team, one lead engineer, and several extra staff members.</p><div className="mt-4 grid gap-3"><select value={assignment?.assignedTeamId ?? ""} onChange={event => update({ assignedTeamId: event.target.value ? Number(event.target.value) : null })} className="rounded-lg border border-slate-300 px-3 py-2 text-sm"><option value="">No team</option>{teams.map(team => <option key={team.id} value={team.id}>{team.name}</option>)}</select><select value={leadStaffMemberId ?? ""} onChange={event => updateLead(event.target.value ? Number(event.target.value) : null)} className="rounded-lg border border-slate-300 px-3 py-2 text-sm"><option value="">No lead engineer</option>{members.map(member => <option key={member.id} value={member.id}>{member.firstName} {member.lastName}</option>)}</select><div><p className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Extra staff</p><div className="flex flex-wrap gap-2">{staffOptions.map(member => <button key={member.id} type="button" onClick={() => toggleStaff(member.id)} className={`rounded-full border px-3 py-1 text-xs font-semibold ${selectedStaffIds.includes(member.id) ? "border-blue-600 bg-blue-600 text-white" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-50"}`}>{member.firstName} {member.lastName}</button>)}{staffOptions.length === 0 && <span className="text-sm text-slate-500">No extra staff available.</span>}</div></div>{saving && <p className="text-xs text-slate-500">Saving...</p>}</div></section>;
}

        ...
        <div className="flex min-h-[700px] flex-col border-r border-gray-200 bg-white last:border-r-0">
            <div className="border-b border-gray-200 bg-gray-50 p-3">
                <div className="flex justify-between">
                    ...
                    <div className="text-right text-xs font-bold text-gray-700">{load} / {MAX_JOBS_PER_DAY}</div>
                </div>
            ...
            <div className="flex-1 space-y-2 p-2">
                {leaveItems.map(item => <div key={`leave-${item.id}`} className="rounded border border-amber-300 bg-amber-50 p-2 text-xs shadow-sm"><div className="font-semibold text-amber-900">Annual leave</div><div className="text-amber-800">{item.staffName}</div><div className="mt-1 text-[10px] font-medium text-amber-700">{item.status}{item.reason ? ` · ${item.reason}` : ""}</div></div>)}
                ...
                {jobs.map(job => {
                    ...
                    return <button key={job.id} type="button" onClick={() => onSelectJob(job)} className="w-full rounded border p-2 text-left text-xs shadow-sm transition hover:shadow" style={{ borderColor: teamColour.hex, backgroundColor: teamColour.softHex }}><div className="font-semibold text-gray-900">{job.jobTitle}</div><div className="text-gray-500">{job.customer}</div><div className="mt-2 flex flex-wrap gap-1"><span className="rounded bg-orange-100 px-1.5 py-0.5 text-[10px] font-medium text-orange-700">{job.priority}</span><span className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-700">{job.status === "InProgress" ? "In Progress" : job.status}</span></div>{job.assignedTeamName && <div className="mt-1 text-[10px] font-semibold" style={{ color: teamColour.hex }}>{job.assignedTeamName}</div>}<div className="mt-1 text-[10px] text-blue-500">{getEngineerName(job.engineerId)}</div></button>;
                })}
            </div>
        </div>
    );
